...
class JournalEntryResource extends Resource
{
    ...
    {
        $user = auth()->user();
        return $user && ($user->hasPermissionTo('view_journal_entries') || $user->hasRole('super_admin'));
    }

    public static function canCreate(): bool
    {
        $user = auth()->user();
        return $user && ($user->hasPermissionTo('create_journal_entries') || $user->hasRole('super_admin'));
    }

    public static function canEdit($record): bool
    {
        $user = auth()->user();
        return $user && ($user->hasPermissionTo('edit_journal_entries') || $user->hasRole('super_admin'));
    }

    public static function canDelete($record): bool
    {
        $user = auth()->user();
        return $user && ($user->hasPermissionTo('delete_journal_entries') || $user->hasRole('super_admin'));
    }
